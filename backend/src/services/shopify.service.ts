...
import { ShopifyOrder, ShopifyLineItem } from '../types/index.js';

// Ensure we're using the .myshopify.com domain for API calls
const getShopifyDomain = (domain: string): string => {
...
async function enrichLineItemsWithImages(lineItems: ShopifyLineItem[]): Promise<void> {
...
export async function getOrderByGid(gid: string): Promise<ShopifyOrder> {
...
export async function listRecentOrders(limit: number = 10): Promise<ShopifyOrder[]> {
  try {
    const response = await shopify.get<{ orders: ShopifyOrder[] }>('/orders.json', {
      params: {
        limit,
        status: 'any',
