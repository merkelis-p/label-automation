...
export async function mockFulfillment(req, res) {
    try {
        ...
        // Extract locker ID
        const lockerAttr = fulfillment.note_attributes.find(attr => attr.name === 'DPD parcel locker' || attr.name === 'omniva_parcel_terminal');
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                const printJob = {
                    ...
                    status: (printerStatus && printerStatus.status === 'online') ? 'printing' : 'queued',
                    labelPath: updatedOrder.labelPath,
                    createdAt: new Date().toISOString(),
                };
...
export async function seedTestData(req, res) {
    try {
        // Clear existing data
        store.clear();
        ...
        const testOrders = recentOrders.map((shopifyOrder, index) => {
            const carriers = ['OMNIVA', 'DPD_LT'];
            const statuses = ['label_created', 'printed', 'pending'];
...
export async function startAutoGenerate(req, res) {
    ...
    const intervalSeconds = parseInt(req.query.interval) || 30;
...
export async function stopAutoGenerate(req, res) {
    if (autoGenerateInterval) {
...
export async function togglePrinterStatus(req, res) {
...
export function processQueuedJobs() {
    ...
    queuedJobs.forEach(job => {
        ...
        // Update to printing
        const updatedJob = { ...job, status: 'printing' };
...
export async function mockMakeCommerceLabel(req, res) {
    res.json({
        success: true,
        labelUrl: 'https://static.maksekeskus.ee/lbl/mock-label.pdf',
    });
}
export async function mockPrintNodeJob(req, res) {
    ...
    const matchingOrder = orders.find(order => order.labelUrl === content || order.labelPath === content);
    if (!matchingOrder) {
        return res.status(404).json({
            success: false,
